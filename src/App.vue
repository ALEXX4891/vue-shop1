...
export default {
  ...
  created() { // хук - создание компонента
    const userAccessKey = localStorage.getItem('userAccessKey'); // получение ключа из localStorage
    if (userAccessKey) {
      this.updatUserAccessKey(userAccessKey);
    }
    this.loadCart(); // вызов даействия из глобального хранилища store
  },
  methods: {
    ...mapActions(['loadCart']), // проксирование действия из глобального хранилища store в методы данного компонента
    ...mapMutations(['updatUserAccessKey']),
  },
};
</script>
